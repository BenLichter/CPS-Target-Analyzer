...
function PipelineTab({ deals, setDeals, history, onViewResult, tKey, njKey, onOpenBulkUpload }) {
  ...
  var s21 = useState(null); var bulkFinProgress = s21[0]; var setBulkFinProgress = s21[1];
  ...
  async function bulkUpdateFinancials(vertical) {
    ...
    setBulkFinProgress({ current: 0, total: targets.length, done: [], failed: [] });
    for (var i = 0; i < targets.length; i++) {
      var deal = targets[i];
      setBulkFinProgress(function(p){ return Object.assign({}, p, { current: i + 1, company: deal.company }); });
      try {
        ...
        setBulkFinProgress(function(p){ return Object.assign({}, p, { done: p.done.concat([deal.company]) }); });
      } catch(e) {
        setBulkFinProgress(function(p){ return Object.assign({}, p, { failed: p.failed.concat([deal.company]) }); });
      }
      if (i < targets.length - 1) await new Promise(function(r){ setTimeout(r, 1200); });
    }
    setBulkFinProgress(function(p){ return Object.assign({}, p, { complete: true }); });
                ...
                <div onClick={function(){ setPipeView({vertical:pipeView.vertical,tier:"all"}); setShowAdd(false); setSegFilter("all"); }}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                      ...
                      {!bulkFinProgress && (
                        <button onClick={function(e){ e.stopPropagation(); if(window.confirm("Rerun financial calculations for all "+activeVert.label+" accounts? This will update ARR for all "+deals.filter(function(d){return d.vertical===pipeView.vertical;}).length+" deals.")) bulkUpdateFinancials(pipeView.vertical); }}
                          style={{ background:C.surface, border:"1px solid #F59E0B55", color:"#F59E0B", borderRadius:5, padding:"3px 8px", fontSize:9, fontWeight:700, cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap" }}>
                          📐 Rerun All Financials
                        </button>
                      )}
                      {bulkFinProgress && (
                        <div onClick={function(e){e.stopPropagation();}} style={{ display:"flex", alignItems:"center", gap:6, background:C.surface, border:"1px solid #F59E0B44", borderRadius:5, padding:"3px 10px" }}>
                          <span style={{ color:"#F59E0B", fontSize:9, fontWeight:700 }}>
                            {bulkFinProgress.complete ? "✅ Done — "+bulkFinProgress.done.length+" updated" : "📐 "+bulkFinProgress.current+"/"+bulkFinProgress.total+" — "+( bulkFinProgress.company||"")}
                          </span>
                          {!bulkFinProgress.complete && (
                            <div style={{ width:60, height:4, background:C.border, borderRadius:99, overflow:"hidden" }}>
                              <div style={{ width:Math.round(bulkFinProgress.current/Math.max(bulkFinProgress.total,1)*100)+"%", height:"100%", background:"#F59E0B", borderRadius:99, transition:"width 0.4s" }} />
                            </div>
                          )}
                        </div>
                      )}
