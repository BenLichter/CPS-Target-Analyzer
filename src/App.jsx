...
async function runAnalysis(company, onStep, keys) {
  ...
  const SYS = 'You are a senior B2B sales intelligence expert for CoinPayments (100+ digital assets, white-label infrastructure, fiat on/off ramps, API-first). Output ONLY valid JSON. No markdown. Start with { end with }. Values under 35 words.\n\nARR METHODOLOGY — follow exactly:\nStep 1: Identify the company primary scale metric from the provided sources — pick the most granular one available: active users, monthly transaction volume, annual payment volume, AUM, or GMV. Use the specific number from the provided Tavily and NinjaPear data; do not substitute generic industry estimates.\nStep 2: Based on the company specific sub-vertical and business model, select the crypto penetration rate that best reflects their user base\'s crypto affinity. Use your knowledge of real adoption rates by sector — e.g. remittance fintechs have high crypto demand (12-18%), trading platforms are already crypto-adjacent (8-15%), neobanks moderate (4-8%), regional banks low (2-5%), escrow/title moderate (3-6%), luxury travel/goods low (2-5%), gaming/betting high (15-25%). Adjust within the range based on the company\'s specific signals in the provided data. Justify your chosen rate in the assumptions.\nStep 3: Addressable base = scale metric × chosen penetration rate. SOM = addressable base × average revenue per unit. Use actual ARPU if visible in sources; otherwise default: consumer fintech $450/user/yr, brokerage/investment $1,200/user/yr, mass-market app $200/user/yr, B2B platform $800/account/yr.\nStep 4: projected_arr = SOM × capture rate. Use 1-1.5% for companies with no current crypto exposure (early stage). Use 1.5-2% for companies already exploring or piloting crypto payments. upside_arr = SOM × 3%.\nStep 5: Output fields: tam = broad industry market size (reference only), som = bottoms-up addressable market per above, projected_arr = SOM × capture rate, upside_arr = SOM × 3%. Always show full math inline e.g. "15M users × 6% = 900K users × $450 ARPU = $405M SOM × 1.5% = $6.1M ARR".';
